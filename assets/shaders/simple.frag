...
layout(binding = 5, std430) readonly buffer texutres
{
    sampler2D textures[];
};
...
vec3 calc_point(vec3 frag_pos, vec3 view_pos, vec3 n)
{
    ...
    float spec = pow(max(dot(n, half_way), 0.0), point_light_specular_power) * texture(textures[specular_tex_index], uv).r;
