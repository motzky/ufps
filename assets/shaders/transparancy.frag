...
void main()
{
    // TODO: Do we need lighting on glass ???
    
    vec4 albedo_texel = texture(sampler2D(albedo_bindless_handle), uv);

    out_color = albedo_texel;
    return;

    // vec3 albedo = albedo_texel.rgb;
    // float alpha = min(albedo_texel.a, opacity);


    // vec3 emissive = vec3(0.0);
    // float emissive_alpha = 1.0;
    // if(emissive_bindless_handle.x < 65535)
    // {
    //     vec4 emissive_texel = texture(sampler2D(emissive_bindless_handle), uv);
    //     emissive = emissive_texel.rgb;
    //     emissive_alpha = min(opacity, emissive_texel.a);
    // }
    ...
    // {
    //     if(emissive_alpha < 0.001)
    //     {
    //         discard;
    //     }
    //     out_color = vec4(emissive, emissive_alpha);
    //     return;
    // }    
    ...
    // {
    //     metallic = texture(sampler2D(specular_bindless_handle), uv).r;
    // }

    // float roughness = 1.0;
    // if(roughness_bindless_handle.x < 65535)
    // {
    //     roughness = texture(sampler2D(roughness_bindless_handle), uv).r;
    // }

    // float ao = 1.0;
    // if(ao_bindless_handle.x < 65535)
    // {
    //     ao = texture(sampler2D(ao_bindless_handle), uv).r;
    // }

    // vec3 view_pos = vec3(camera_position[0],camera_position[1],camera_position[2]);
    // vec3 view_dir = normalize(view_pos - frag_position.xyz);
    ...
    // {
    //     Lo += calculate_point_light(point_lights[i], view_pos, view_dir, normal, frag_position.xyz, albedo, F0, metallic, roughness, ao);
    // }
