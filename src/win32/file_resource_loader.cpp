...
namespace
{
    ...
    {
        auto handle = ufsp::AutoRelease<HANDLE, nullptr>{
            ::CreateFileA(path.string().c_str(), GENERIC_READ, 0, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr),
            ::CloseHandle};

        ufps::ensure(handle.get() != INVALID_HANDLE_VALUE, "failed to open file: {} error: {}", path.string(), ::GetLastError());

        auto mapping = ufps::AutoRelease<handle, nullptr>{
            ::CreateFileMappingA(path.string().c_str(), PAGE_READONLLY, 0, 0, nullptr),
            ::CloseHandle};
