...
namespace ufps
{
    ...
    constexpr BoundedNumber<T, Min, Max>::BoundedNumber(T value)
        : _value{value}
    {
        ensure(_value >= Min && _value <= Max, "{} not in range ({}, {})", _value, Min, Max);
    }
    ...
    template <class T, T Min, T Max>
    template <class U, U OtherMin, U OtherMax>
    constexpr auto BoundedNumber<T, Min, Max>::operator=(const BoundedNumber<U, OtherMin, OtherMax> &other) -> BoundedNumber<T, Min, Max> &
    {
    ...
    template <class T, T Min, T Max>
    template <class U, U OtherMin, U OtherMax>
    constexpr auto BoundedNumber<T, Min, Max>::operator=(BoundedNumber<U, OtherMin, OtherMax> &&other) -> BoundedNumber<T, Min, Max> &
    {
        *this = BoundedNumber<T, Min, Max>{*other};
        return *this;
    }
    ...
    template <class T, T Min, T Max>
    constexpr auto BoundedNumber<T, Min, Max>::operator-() const -> BoundedNumber<T, static_cast<T>(-Max), static_cast<T>(-Min)>
        requires std::is_signed_v<T>
    {
        return BoundedNumber<T, -Max, -Min>{-_value};
    }

    template <class T, T Min, T Max>
    constexpr auto BoundedNumber<T, Min, Max>::operator+() const
    {
        const auto new_value = +_value;
    ...
    template <class T, T TMin, T TMax, class U>
    constexpr auto operator+(U u, const BoundedNumber<T, TMin, TMax> &num1)
    {
        return BoundedNumber<U>{u} + num1;
    }
    ...
    constexpr auto &operator+=(BoundedNumber<T, TMin, TMax> &num1, const BoundedNumber<U, UMin, UMax> &num2)
    {
        ...
        ensure(new_value >= TMin && new_value <= TMax, "{} + {} = {}, out of range ({}, {})", *num1, *num2, new_value, TMin, TMax);
    ...
    template <class T, T TMin, T TMax, class U>
    constexpr auto operator-(const BoundedNumber<T, TMin, TMax> &num1, U u)
    {
        const auto new_value = *num1 - u;
        using NewType = decltype(new_value);
    ...
    constexpr auto operator*(const BoundedNumber<T, TMin, TMax> &num1, const BoundedNumber<U, UMin, UMax> &num2)
    {
        ...
        return BoundedNumber<
            NewType,
            std::min({static_cast<NewType>(TMin) * static_cast<NewType>(UMin),
                      static_cast<NewType>(TMin) * static_cast<NewType>(UMax),
                      static_cast<NewType>(TMax) * static_cast<NewType>(UMin),
                      static_cast<NewType>(TMax) * static_cast<NewType>(UMax)}),
            std::max({static_cast<NewType>(TMin) * static_cast<NewType>(UMin),
                      static_cast<NewType>(TMin) * static_cast<NewType>(UMax),
                      static_cast<NewType>(TMax) * static_cast<NewType>(UMin),
                      static_cast<NewType>(TMax) * static_cast<NewType>(UMax)})>{new_value};
    }
    ...
    constexpr auto operator*(const BoundedNumber<T, TMin, TMax> &num1, U u)
    {
        ...
        return BoundedNumber<NewType, std::numeric_limits<NewType>::lowest(), std::numeric_limits<NewType>::max()>{new_value};
    }
    ...
    template <std::uint32_t Min = std::numeric_limits<std::uint32_t>::lowest(), std::uint32_t Max = std::numeric_limits<std::uint32_t>::max()>
