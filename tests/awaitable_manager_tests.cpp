...
TEST(awaitable_manager, await_next_tick)
{
    ...
    [[maybe_unused]] const auto coro = [](ufps::AwaitableManager &am, int &x) -> ufps::EagerTask
    {
        co_await am;
        x = 1;
    }(am, x);
...
TEST(awaitable_manager, await_timer)
{
    ...
    [[maybe_unused]] const auto coro = [](ufps::AwaitableManager &am, std::atomic<int> &x) -> ufps::EagerTask
    {
        co_await am(500ms);
        x = 1;
    }(am, x);
...
TEST(awaitable_manager, next_tick_exception_captured)
{
    ...
    [[maybe_unused]] const auto coro = [](ufps::AwaitableManager &am) -> ufps::EagerTask
    {
        co_await am;
        throw 1;
    }(am);
